...
echo "Setup helm"
sleep 4
curl -fsSL -o get_helm.sh https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3sleep 5
...
kubeadm.sh  provider.tf  provider.tf_bkp  provider.tf_k8s  terraform.tfstate  terraform.tfstate.backup
root@localhost:~/linode# cat kubeadm.sh
#!/bin/bash
sleep_func() {
  local duration=$1
  echo "Sleeping for ${duration} seconds..."
